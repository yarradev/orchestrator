...
- [x] **D3 — RESOLVED (low-risk default; sign-off 2026-07-02): `reduce` emits board `ActInput[]` directly** (no neutral `Op[]`). Design §3.1 names board acts (MOVE/REJECT/VETO/HOLD/…); the neutral `Op` union + `opToAct` were the adaptor seam being deleted. `reduce(verdict, card, lifecycle): ActInput[]`.

- [x] **D4 — RESOLVED (low-risk default): advice/clean → the `ADVICE` act (from Phase 0 P0-1).** `reduce` maps `advice`/`clean` → an `ADVICE` act (the "persisted reviewed-head record"), closing the Phase-0 operational watch-item so a watched-path card can clear `advisor_clear`. In Phase 1 scope.

- [x] **D5 — RESOLVED (low-risk default): keep a test-only fake of the ONE `boardClient`, not a pluggable `BoardBackend`.** Drop `BoardBackend` polymorphism; use a scripted `fetch` / fake client object for `decide`/`reduce`/`runPass` tests.

_(D3/D4/D5 are recorded as the recommended low-risk defaults so a fresh session can execute end-to-end without stopping; override any of them at execution if desired.)_
